...
Aktuálně: **Sprint 3A/B/C/D/E uzavřeno, 3F čeká na build** (2026-04-15 session). Dnešní práce: image upload pipeline (`expo-image-picker` + `expo-image-manipulator` + `expo-file-system` `new File().arrayBuffer()`), storage RLS policies, Claude Vision identifikace přes direct client call s per-device Anthropic API klíčem v `expo-secure-store`, nový `app/(app)/profile.tsx` global settings, Brother print HTML prototype (two-step `printToFileAsync → printAsync(uri)`, dynamic font sizing + center align, QR logo overlay s Python PIL pipeline), **TestFlight build pipeline** (EAS build + ASC record 6762301537 + Internal Testing Family group, app live na iPhonu přes TestFlight), **Brother SDK integrace** přes patchovaný `expo-brother-printer-sdk` (knihovna default supports jen QL, náš patch přidává PT series s `BRLMPTPrintSettings` + `_printerModelFromName` rozšíření o PT-P710BT, `postinstall: patch-package` hook), portrait PDF + CSS rotation fix (Brother SDK treats PDF width jako tape width → landscape → 0.3× scale → miniature; fix: `@page 68pt 227pt` + `.label` rotated 90° via `transform: translate(-50%,-50%) rotate(90deg)`). **STRATEGICKÁ ZMĚNA**: prepper emergency use case vyžaduje offline-first data layer (TestFlight 90-day expiry nekompatibilní) + App Store finální distribuce. Sprint 4' (offline-first refactor: WatermelonDB / PowerSync / Legend-State / custom SQLite research) má přednost před Sprintem 4 (push notifikace). Next: čekat na EAS build (free tier queue ~4h, přes noc) → submit → reálný Brother print test → commit Sprint 3F → rozhodnout Sprint 4' architectural path.
